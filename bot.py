...
async def play(ctx, *, activite: str):
    ...
    print(f"Activite changee en : {activite} par {ctx.author}")


@play.error
async def play_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send("Usage : !play <activite>\nExemple : !play Minecraft")
...
async def limit(ctx, arg: str, nombre: int = None, temps: float = None):
    ...
    if arg.lower() == "on":
        ...
        await ctx.send("Anti-Raid active. Seuil : " + str(config['seuil']) + " salons en " + str(config['fenetre']) + "s")
        print(f"Bot active par {ctx.author}")

    elif arg.lower() == "off":
        config["actif"] = False
        historique_vocaux.clear()
        await ctx.send("Anti-Raid desactive.")
        print(f"Bot desactive par {ctx.author}")
        ...
        print(f"Limites mises a jour par {ctx.author} : {nombre} vocaux / {temps}s")
