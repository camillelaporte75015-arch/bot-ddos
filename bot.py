...
async def play_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send("Utilisation : !play <activite>")
...
async def limit(ctx, *, arg: str = None):
    if not est_owner(ctx):
        await ctx.send("❌ Tu n'as pas la permission d'utiliser cette commande.")
        return

    if arg is None:
        await ctx.send("Utilisation : !limit <nombre>/<seconde>s\nExemple : !limit 3/6s")
        return
    ...
    if arg.lower() == "on":
        ...
        await ctx.send(f"Anti DDOS active, limite de {config['seuil']} par {config['fenetre']}s")
        return
    ...
    if arg.lower() == "off":
        config["actif"] = False
        historique_vocaux.clear()
        await ctx.send("Anti DDOS désactivé.")
        return

    # !limit <nombre>/<seconde>s  ex: 3/6s ou 5/10s
    try:
        if "/" not in arg:
            raise ValueError
        parties = arg.split("/")
        nombre = int(parties[0].strip())
        temps_str = parties[1].strip().lower().replace("s", "")
